...
class BK895App:
    ...
    def set_progress(self, value):
        self.root.after(
            0,
            lambda: self.progress_value.set(value)
        )
    ...
    def query_bk(self, command):
        ...
        # The BK895 uses non-ASCII bytes for some symbols
        # such as ohm. We only need the numeric value and
        # SI prefix, so those bytes can safely be ignored.
        return raw.decode(
            "ascii",
            errors="ignore"
        ).strip()
    ...
    def parse_capacitance(self, value):
        ...
        raise ValueError(
            "Unknown capacitance value: " + repr(value)
        )
    ...
    def fetch_measurement(self):
        ...
        if len(values) < 2:
            raise RuntimeError(
                "Unexpected FETC response: " + repr(response)
            )
        ...
        capacitance = self.parse_capacitance(
            capacitance_text
        )

        resistance = self.parse_resistance(
            resistance_text
        )
    ...
    def excel_value(self, value):
        """
        Excel cells should be empty for invalid
        NaN measurements.
        """
    ...
    def create_excel(
        ...
        for mode, cycles_data in results.items():
            ...
            for point_index in range(
                ...
                for cycle_index, cycle_data in enumerate(
                    cycles_data
                ):
            ...
            chart_r.x_axis.title = (
                "Frequency (Hz)"
            )

            chart_r.y_axis.title = (
                "Resistance (Ohm)"
            )
